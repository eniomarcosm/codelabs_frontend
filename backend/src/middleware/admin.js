...
exports.getAll = (req, resp) => {
    let paramiters = [];
    const sql = "SELECT * FROM usuario WHERE isDeleted=0";
...
exports.getById = (req, resp, next) => {
    ...
    const sql = "SELECT * FROM usuario WHERE idusuario = ? and isDeleted=0";
    con.query(sql, paramiters, (err, res) => {
        if (err) return resp.status(500).send(err);
        return resp.status(200).send(res);
    });
...
exports.save = async (req, resp) => {
    ...
    const sql = "INSERT INTO usuario(email_inst, senha, role_id, createdBy) values(?,?,?,?)";
    con.query(sql, paramiters, (err, res) => {
...
exports.addUserEstudante = (req, resp) => {
    ...
    con.query("insert into usuario_estudante(idusuario, idestudante) values (?,?)", params, (err, res) => {
